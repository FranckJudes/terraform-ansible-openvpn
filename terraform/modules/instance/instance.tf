...
resource "openstack_compute_instance_v2" "instance" {
  ...
  flavor_name       = var.instance_flavor_id
  security_groups = var.instance_security_groups
  key_pair        = var.instance_key_pair
  metadata = var.metadatas
  user_data = data.template_file.userdata.rendered # Pour utiliser cloud-init au lancement

  network {
    name = var.instance_network_internal_name
...
resource "openstack_compute_floatingip_associate_v2" "fip_1" {
  count = var.public_floating_ip == true ? var.instance_count : 0
...
# Liste des noms de périphériques pour les volumes supplémentaires
locals {
